...
### Per-Node Config Fan-Out & Reply Contract
Monitor-configuration updates fan out to every cluster node and collect a per-node outcome rather than treating the cluster as all-or-nothing. When adding or changing this flow:

* **Shared types live in `valkey-common`.** `NodeResult` (`{ nodeId, success, message }`), `ReplyOutcome` (`"fulfilled" | "failed" | "not_attempted"`), `NodeResultsReply` (`{ outcome, nodeResults, notAttemptedNodeIds?, appliedConfig? }`), and the per-node status types (`NodeRetryStatus`, `NodeStatusUpdate`, `NodeStatusPush`) are defined once in `common/src/node-results.ts` and imported by both the Server and the Frontend. Do not redefine these shapes.
* **Fan-out primitives.** `apps/server/src/actions/node-fanout.ts` holds the target/result shapes and the bounded single-attempt runner (`runNodeAttempt`, 5000 ms per-node timeout, never rejects) plus outcome classification (`toOutcome`). Strategy layers compose these; keep scheduling policy out of that file.
* **Automatic retry.** Config updates run through the modular retry runner in `apps/server/src/actions/retry-runner.ts` (`runWithRetry`): Fibonacci backoff (shared `retryDelay`), hard cap of 6 retries per node, a required live status callback per node transition, and abort support. The runner is policy-agnostic — feature-specific overrides are resolved by the caller and passed as plain options (the config feature reads `CONFIG_RETRY_MAX_RETRIES` / `CONFIG_RETRY_DELAYS_MS` in `config.ts`); a new consumer supplies its own. `config.ts` keeps one session per target id — a new `config/updateConfig` for the same target aborts the in-flight session (its aggregate reply is suppressed and its monitor rider is skipped). Reuse `runWithRetry` when adding retry to other fan-outs.
* **Reply id-space.** Replies carry exactly one arm of `AggregateReplyId` (`{ clusterId }` for a cluster or the db-less `{ nodeId }` for a standalone node), never both. A standalone reply contains exactly one `NodeResult` whose `nodeId` equals the reply's `nodeId`. Use `toNodeId` for the single shared db-strip.
* **Websocket actions.** The config push session is driven by `config/updateConfig` (`{ connectionId, clusterId?, config, monitorAction? }`). The optional `monitorAction` rider starts/stops MONITOR on exactly the config-succeeded nodes after the session resolves (total failure or a superseded session skips the toggle). Pure toggles with no config change use `monitor/monitorRequested` directly. The server pushes `config/updateConfigNodeStatus` on every node transition (attempting/retrying/succeeded/failed/not_attempted) and sends one aggregate `config/updateConfigFulfilled` / `config/updateConfigFailed` (carrying `outcome`, `nodeResults`, `notAttemptedNodeIds`) when the session resolves. Retry is server-side only — the frontend never sends retry requests.
