...
- **Learned count:** successful connects (standalone and cluster) include a `databasesCount` field in the `standaloneConnectFulfilled` / `clusterConnectFulfilled` payload's connection details, which the frontend stores and the Edit Connection form uses to display and enforce the valid range for that server. The field is **omitted entirely** when the count could not be read (e.g. `CONFIG GET` denied by ACL).
...
- **Validation:** before dispatching `connectPending` or `discoveryEndpointPending`, the form checks that `db` is a non-negative integer (`isValidDatabaseIndex` from `common/src/connection-id.ts`) and surfaces an inline error under the Database field if not. The Edit Connection form additionally checks the value against the server's learned `databasesCount` and shows the valid range as a hint under the field. That count only describes the server it was learned from, so both the hint and the bound disappear as soon as the form's host or port is edited away from the connection's current values — the new server's range is unknown until it is connected to.
