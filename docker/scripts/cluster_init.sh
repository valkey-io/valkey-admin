...
if ! out=$(
  valkey-cli --cluster create \
    "$HOST:7000" "$HOST:7001" "$HOST:7002" "$HOST:7003" "$HOST:7004" "$HOST:7005" \
...
while [ "$i" -lt 120 ]; do
  ...
  masters="$(valkey-cli -h "$HOST" -p 7000 CLUSTER NODES 2>/dev/null | tr -d '\r' | awk '$3 ~ /master/ {c++} END {print (c+0)}' || echo 0)"
