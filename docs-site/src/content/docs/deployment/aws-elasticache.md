---
title: AWS ElastiCache Deployment
description: Deploy Valkey Admin on AWS to monitor an ElastiCache Valkey cluster
---

Deploy Valkey Admin on AWS to monitor an ElastiCache Valkey cluster with HTTPS, Cognito authentication, and full network isolation.
...
- **ALB (Application Load Balancer):** Provides internet-facing HTTPS access. Keeps the EC2 instance in a private subnet with no public IP, and integrates with Amazon Cognito for user authentication.
- **Amazon Cognito:** Handles user authentication. Users must sign in before accessing Valkey Admin. Add users through the AWS Console or CLI after deployment.
- **EC2 instance:** Runs the Valkey Admin Docker container in a private subnet. Outbound internet access is via a NAT Gateway for pulling the Docker image and system updates.
- **ElastiCache cluster:** The Valkey cluster being monitored, in a private subnet. Accessible only from the EC2 instance.

### Prerequisites

- AWS CLI configured with appropriate permissions
- A domain name with a DNS record pointing to the ALB (required for HTTPS)
- An ACM certificate for your domain (available via the AWS Certificate Manager console)

### CloudFormation Template

The template below creates a complete deployment:
...
- All traffic is encrypted via HTTPS (HTTP requests redirect to HTTPS)
- Users must authenticate through Amazon Cognito before accessing the application
- The EC2 instance has **no public IP** — accessible only through the ALB
...
- [Docker deployment](/deployment/docker/) for simpler setups
- [Kubernetes deployment](/deployment/kubernetes/) for container-native deployments
- [Configuration reference](/configuration/server/) for all environment variables
